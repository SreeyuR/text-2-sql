...
from agent_instruction_generator import analyze_csv_files, analyze_json_gz_files, generate_instruction, invoke_claude_3_with_text

from Prep_Data import get_s3_data_sources, list_s3_folders, prep_data
...
class MyStack(Stack):
    def __init__(self, scope: App, id: str, region: str, **kwargs):
        ...
        # Reference an existing S3 bucket with the data by ARN (Amazon Resource Name)
        bucket_name = "denali-ddb-export-ofudrlp62bhbtaf2caatagswfu-poc-us-east-1"
        athena_result_loc = f"athena-destination-store-texttosql"

        schema_bucket = s3.Bucket.from_bucket_arn(self, "SchemaBucket", f"arn:aws:s3:::{bucket_name}")
        athena_output_bucket = s3.Bucket.from_bucket_arn(
            self, 
            "AthenaOutputBucket", 
        ...
        crawler_name = "Denali_AI_Chatbot_Crawler"  # Define the crawler name as a variable
        glue.CfnCrawler(
            self,
            crawler_name,
            ...
            schedule=glue.CfnCrawler.ScheduleProperty(
                # schedule the crawler to run every minute
        ...
        action_group_function = lambda_.Function(
            self,
            "ActionGroupFunction",
            runtime=lambda_.Runtime.PYTHON_3_9, # python 3.9
            handler="index.lambda_handler",
            code=lambda_.Code.from_asset("./lambda/agent/"), # aws packages this directory & deploys to Lambda
            environment={
              'outputLocation': 's3://'+athena_result_loc+'/',
        ...
        action_group_function.role.add_to_policy(iam.PolicyStatement(
            actions=[
                ...
                "glue:UpdatePartition", "glue:BatchDeletePartition", "glue:GetTable", "glue:GetDatabase", 
                "glue:GetTables"
            ],
        ...
        action_group_function.role.add_to_policy(iam.PolicyStatement(
            ...
            resources=[f"{schema_bucket.bucket_arn}", f"{schema_bucket.bucket_arn}/*", f"{athena_output_bucket.bucket_arn}", f"{athena_output_bucket.bucket_arn}/*"]
        ))
        action_group_function.role.add_to_policy(iam.PolicyStatement(
            actions=[
        ...
        top_level_s3_folders = list_s3_folders(schema_bucket.bucket_name)
        data_context = analyze_json_gz_files(schema_bucket.bucket_name, top_level_s3_folders, region)

        instruction_text = generate_instruction(database_name="denali_ai", data_context=None, env=ENV) #denali_ai_chatbot for database if you want to infer
        # question = f"""
        ...
        def generate_unique_id(prefix="agent"):
            ...
            random_suffix = ''.join(random.choices(string.ascii_lowercase + string.digits, k=6))  # Random string
        ...
        agent = Agent(
            ...
            name='SQL-Agent-CDK',
            # memory_enabled = False  # Disable memory (not a valid parameter)
        )
...
MyStack(app, "text-2-sql-v3", region=region)
app.synth()
